...
impl ChannelData {
    fn new(info: ChannelInfo, dbc: Option<Arc<ParsedDbc>>) -> Self {
        Self { frames: VecDeque::new(), signals: HashMap::new(), dbc, info }
    }
    ...
    fn push(
        ...
        frame: StoredFrame,
        window_ms: u64,
    ) -> Vec<(String, f64, String, String, f64, f64)> {
    ...
    fn rescan(&mut self, sig_name: &str) {
        let vals: Vec<f64> = self.frames.iter()
    ...
    fn get_frames(&self, limit: usize) -> Vec<FrameInfo> {
        ...
        self.frames.iter().skip(skip).map(|f| FrameInfo {
        ...
        self.frames.iter()
            ...
            .filter_map(|f| {
                f.signals.iter()
                    ...
                    .map(|s| SignalSample { timestamp_ms: f.timestamp_ms, value: s.value })
            })
            .collect()
    }
}
...
impl CanManager {
    pub fn new(app_state: Arc<AppState>) -> Self {
        ...
        #[cfg(feature = "kvaser")]
        {
            ...
            cans.insert("kvaser".to_string(), Can::new(
                KvaserBackend,
        ...
        {
            #[cfg(target_os = "linux")]
            let get_pw = { let s = Arc::clone(&app_state); move || s.get_sudo_password() };
            #[cfg(not(target_os = "linux"))]
            let get_pw = || -> Result<String, String> { Err("sudo not supported".to_string()) };

            let sh_rx = Arc::clone(&shared);
            let sh_tx = Arc::clone(&shared);
            cans.insert("socketcan".to_string(), Can::new(
    ...
    pub fn open_channel(
        ...
        let existing_index = self.shared.lock()
        ...
        let can = self.cans.get_mut(&backend_name)
            .ok_or_else(|| format!("No backend '{backend_name}'"))?;
        let hw_index = can.list_channels()
            .iter()
            .position(|n| n == &channel_name)
            .ok_or_else(|| format!("Channel '{channel_name}' not found in '{backend_name}'"))?
            as u8;
    ...
    pub fn close_channel(&mut self, channel_id: &str) -> Result<(), String> {
        let (backend_name, hw_index) = {
            let mut lock = self.shared.lock().map_err(|_| "Lock poisoned".to_string())?;
            let (bn, idx) = lock.id_to_index.remove(channel_id)
    ...
    pub fn send_raw(
        ...
        can.send_once(hw_index, RawFrame { can_id, is_extended: can_id > 0x7FF, data })
    }
    ...
    pub fn get_frames(&self, channel_id: Option<&str>, limit: usize) -> Vec<FrameInfo> {
        let lock = match self.shared.lock() { Ok(l) => l, Err(_) => return Vec::new() };
        if let Some(id) = channel_id {
            lock.channels.get(id).map(|c| c.get_frames(limit)).unwrap_or_default()
        } else {
            let mut all: Vec<FrameInfo> = lock.channels.values()
                .flat_map(|c| c.get_frames(limit))
                .collect();
    ...
    move |hw_index, raw| {
        ...
        let (app, frame_event) = {
            ...
            let stored = StoredFrame {
                ...
                signals: signals.iter().map(|(n, v, u)| StoredSignal {
...
fn decode_frame(dbc: &ParsedDbc, can_id: u32, data: &[u8]) -> Option<Vec<(String, f64, String)>> {
    ...
    let signals = msg.signals.iter().map(|sig| {
        let v = crate::dbc_parser::decode(
            data, sig.start_bit, sig.length, sig.little_endian, sig.signed, sig.factor, sig.offset,
        );
