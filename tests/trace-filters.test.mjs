...
test("visible filter activation and context menu share behavior without sorting or dragging", () => {
    ...
    th.child.listeners.click({ stopPropagation() {} });
    assert.equal(h.ctxMenu, null, "second click closes this button's menu");
    assert.equal(menu.removed, true);
    th.child.listeners.click({ stopPropagation() {} });
    assert.equal(h.ctxMenu, menu, "third click reopens the menu");
    h.ctxMenu = element();
    th.child.listeners.click({ stopPropagation() {} });
    assert.equal(h.ctxMenu, menu, "a different open menu does not toggle this button off");
    ...
    th.child.listeners.click({ stopPropagation() {} });
    assert.equal(h.ctxMenu, null, "button also closes its menu opened by right-click");
    stopped = false;
